...
const StudentPaymentHistory: React.FC = () => {
    ...
    const fetchPayments = async () => {
      ...
      try {
        const response = await fetch(
          `https://665fbf245425580055b0b23d.mockapi.io/payments?userId=${userId}`
        );
        if (!response.ok) {
  ...
  return (
    <div className="container mx-auto mt-10">
      ...
      <Table columns={columns} dataSource={payment || []} rowKey="paymentId" />{" "}
      {/* Use payment || [] to ensure dataSource is an array */}
      {renderComponent()}
    </div>
  );
};
